...
(
    ...
    COMPRESSION_ALGORITHM           char(64)                      not null comment 'Compression algorithm used for data transfer between master and slave.',
    ZSTD_COMPRESSION_LEVEL          int                           not null comment 'Compression level associated with zstd compression algorithm.',
    TLS_CIPHERSUITES                text                          null,
...
create unique index `PRIMARY`
    on performance_schema.replication_connection_configuration (CHANNEL_NAME)
    using hash;
