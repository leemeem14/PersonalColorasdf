...
create unique index `PRIMARY`
    on performance_schema.events_statements_summary_by_program (OBJECT_TYPE, OBJECT_SCHEMA, OBJECT_NAME)
    using hash;

alter table performance_schema.events_statements_summary_by_program
    add primary key (OBJECT_TYPE, OBJECT_SCHEMA, OBJECT_NAME) using hash;
