...
with recursive `role_graph` (`c_parent_user`, `c_parent_host`, `c_from_user`, `c_from_host`, `c_to_user`, `c_to_host`,
                   ...
                   as (select internal_get_username()                        AS `INTERNAL_GET_USERNAME()`,
                       ...
                       where concat(quote(`mandatory_roles`.`ROLE_NAME`), '@',
                             ...
                             (select concat(convert(quote(`role_edges`.`FROM_USER`) using utf8mb4), '@',
                                            convert(quote(`role_edges`.`FROM_HOST`) using utf8mb4))
                              from `role_edges`
                              where ((`role_edges`.`TO_USER` = internal_get_username()) and
                                     (convert(`role_edges`.`TO_HOST` using utf8mb4) =
                       ...
                       select `role_graph`.`c_parent_user`     AS `c_parent_user`,
                              ...
                              `role_edges`.`FROM_USER`         AS `FROM_USER`,
                              `role_edges`.`FROM_HOST`         AS `FROM_HOST`,
                              `role_edges`.`TO_USER`           AS `TO_USER`,
                              `role_edges`.`TO_HOST`           AS `TO_HOST`,
                              if((locate(sha2(concat(convert(quote(`role_edges`.`FROM_USER`) using utf8mb4), '@',
                                                     convert(quote(`role_edges`.`FROM_HOST`) using utf8mb4)), 256),
                                         `role_graph`.`role_path`) = 0), concat(`role_graph`.`role_path`, '->', convert(
                                      sha2(concat(convert(quote(`role_edges`.`FROM_USER`) using utf8mb4), '@',
                                                  convert(quote(`role_edges`.`FROM_HOST`) using utf8mb4)), 256) using
                                      utf8mb4)),
                              ...
                              `role_edges`.`WITH_ADMIN_OPTION` AS `WITH_ADMIN_OPTION`,
                              if(((0 <> `role_graph`.`c_enabled`) or
                                  (0 <> internal_is_enabled_role(`role_edges`.`FROM_USER`, `role_edges`.`FROM_HOST`))),
                                 true,
                                 false)                        AS `IF(c_enabled OR        INTERNAL_IS_ENABLED_ROLE(FROM_USER, FROM_HOST), TRUE, FALSE)`
                       from (`role_edges` join `role_graph`)
                       where ((`role_edges`.`TO_USER` = `role_graph`.`c_from_user`) and
                              (convert(`role_edges`.`TO_HOST` using utf8mb4) = `role_graph`.`c_from_host`) and
...
select distinct `role_graph`.`c_parent_user`                                                                        AS `USER`,
                ...
                (select if(count(0), 'YES', 'NO')
                 from `default_roles`
                 where ((`default_roles`.`DEFAULT_ROLE_USER` = `role_graph`.`c_from_user`) and
                        (convert(`default_roles`.`DEFAULT_ROLE_HOST` using utf8mb4) = `role_graph`.`c_from_host`) and
                        (`default_roles`.`USER` = `role_graph`.`c_parent_user`) and
