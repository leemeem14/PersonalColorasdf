...
from (((((`parameters` `prm` join `routines` `rtn` on ((`prm`.`routine_id` = `rtn`.`id`))) join `schemata` `sch`
         on ((`rtn`.`schema_id` = `sch`.`id`))) join `catalogs` `cat`
        on ((`cat`.`id` = `sch`.`catalog_id`))) join `collations` `col`
       on ((`prm`.`collation_id` = `col`.`id`))) join `character_sets` `cs` on ((`col`.`character_set_id` = `cs`.`id`)))
