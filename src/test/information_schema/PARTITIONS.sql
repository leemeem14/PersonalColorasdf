...
from ((((((`tables` `tbl` join `schemata` `sch` on ((`sch`.`id` = `tbl`.`schema_id`))) join `catalogs` `cat`
          on ((`cat`.`id` = `sch`.`catalog_id`))) left join `table_partitions` `part`
         on ((`part`.`table_id` = `tbl`.`id`))) left join `table_partitions` `sub_part`
        on ((`sub_part`.`parent_partition_id` = `part`.`id`))) left join `tablespaces` `part_ts`
       on ((`part_ts`.`id` = `part`.`tablespace_id`))) left join `tablespaces` `sub_part_ts`
