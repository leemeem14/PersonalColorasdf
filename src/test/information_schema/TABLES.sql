...
from (((((`tables` `tbl` join `schemata` `sch` on ((`tbl`.`schema_id` = `sch`.`id`))) join `catalogs` `cat`
         on ((`cat`.`id` = `sch`.`catalog_id`))) left join `collations` `col`
        on ((`tbl`.`collation_id` = `col`.`id`))) left join `tablespaces` `ts`
       on ((`tbl`.`tablespace_id` = `ts`.`id`))) left join `table_stats` `stat`
