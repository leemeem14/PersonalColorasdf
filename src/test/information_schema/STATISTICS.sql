...
from (((((((`index_column_usage` `icu` join `indexes` `idx` on ((`idx`.`id` = `icu`.`index_id`))) join `tables` `tbl`
           on ((`idx`.`table_id` = `tbl`.`id`))) join `columns` `col`
          on ((`icu`.`column_id` = `col`.`id`))) join `schemata` `sch`
         on ((`tbl`.`schema_id` = `sch`.`id`))) join `catalogs` `cat`
        on ((`cat`.`id` = `sch`.`catalog_id`))) join `collations` `coll`
       on ((`tbl`.`collation_id` = `coll`.`id`))) left join `index_stats` `stat`
