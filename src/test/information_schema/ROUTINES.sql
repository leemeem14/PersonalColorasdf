...
from ((((((((`routines` `rtn` join `schemata` `sch` on ((`rtn`.`schema_id` = `sch`.`id`))) join `catalogs` `cat`
            on ((`cat`.`id` = `sch`.`catalog_id`))) join `collations` `coll_client`
           on ((`coll_client`.`id` = `rtn`.`client_collation_id`))) join `character_sets` `cs_client`
          on ((`cs_client`.`id` = `coll_client`.`character_set_id`))) join `collations` `coll_conn`
         on ((`coll_conn`.`id` = `rtn`.`connection_collation_id`))) join `collations` `coll_db`
        on ((`coll_db`.`id` = `rtn`.`schema_collation_id`))) left join `collations` `coll_result`
       on ((`coll_result`.`id` = `rtn`.`result_collation_id`))) left join `character_sets` `cs_result`
