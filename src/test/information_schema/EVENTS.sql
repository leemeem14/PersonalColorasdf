...
from ((((((`events` `evt` join `schemata` `sch` on ((`evt`.`schema_id` = `sch`.`id`))) join `catalogs` `cat`
          on ((`cat`.`id` = `sch`.`catalog_id`))) join `collations` `coll_client`
         on ((`coll_client`.`id` = `evt`.`client_collation_id`))) join `character_sets` `cs_client`
        on ((`cs_client`.`id` = `coll_client`.`character_set_id`))) join `collations` `coll_conn`
       on ((`coll_conn`.`id` = `evt`.`connection_collation_id`))) join `collations` `coll_db`
